...
/*async function bank() 
pconst { readdirSync, readFileSync, writeFileSync, existsSync, copySync } = require('fs-extra');
const { join, resolve } = require('path');
const pathData = join(__dirname + './modules/data/mbank.json');
const user = require('./modules/data/mbank.json');
...
const login = require("./includes/megu-src")
const axios = require("axios");
const listPackage = JSON.parse(readFileSync('./package.json')).dependencies;
const listbuiltinModules = require("module").builtinModules;
const cof = require("./config.json")

global.thanhdz = new Object({
  altp_url: "https://apiuwuapi.ducdz999.repl.co/altp_data"
})
//global.key = new Object({ownerkey: new Array()})})
...
global.control = require("./ct.js")
global.nodemodule = new Object();
global.money_storage = require("./includes/database/storage/data.json")
...
// cf = Math.round(process.uptime() + cf.UPTIME);
	var patconfig = "./config.json"
	let updata = JSON.parse(readFileSync(patconfig));
updata["UPTIME"] = u + process.uptime();
  
 // console.log(Math.round(cf.UPTIME))
	writeFileSync(global.client.configPath, JSON.stringify(updata, null, 4), "utf-8");
  return
}
setInterval(l,1000*60*60*15);
/////////////////////////////////////////
//========= Load language use =========//
/////////////////////////////////////////
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
     if (loginError) {logger("Auto get appstate","[ SYSTEM ]"); return rb.getapp()}
